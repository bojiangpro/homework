...
def train_PG(exp_name='',
    ...
    # Observation and action sizessy_adv_n
    ...
    if discrete:
        # YOUR_CODE_HERE
        sy_logits_na = build_mlp(sy_ob_no, ac_dim, "hw2_policy", n_layers=n_layers, size=size)
    ...
    if nn_baseline:
        ...
        bl_loss = tf.losses.mean_squared_error(labels=sy_bl_n, predictions=baseline_prediction)
    ...
    for itr in range(n_iter):
        ...
        def q_per_path():
            if reward_to_go:
                for p in paths:
                    rwds = p["reward"]
                    weights = np.power(gamma, range(0, len(rwds)))
                    q_t = np.multiply(weights, rwds)
                    q_t = np.add.accumulate(reversed(q_t))
                    yield q_t[::-1]
            else:
                for p in paths:
                    rwds = p["reward"]
        ...
        q_n = np.concatenate(list(q_per_path()))
        ...
        if nn_baseline:
            ...
            # YOUR_CODE_HERE
            normal_q_n = normalize(q_n, 0, 1)
            sess.run(baseline_update_op, feed_dict={sy_ob_no: ob_no, sy_bl_n: normal_q_n})
        ...
        # YOUR_CODE_HERE
        sess.run(update_op, feed_dict={sy_adv_n: adv_n, sy_ob_no: ob_no, sy_ac_na: ac_na})
